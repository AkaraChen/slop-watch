...
edict: {
		zh: {
			description: `Edict 把 OpenClaw 多智能体流程包装成三省六部朝廷：太子路由、规划与复核衙门，以及看板后面的专职「部」agent。GitHub 仓库 cft0808/edict 以 Python 为主，硬对标 CrewAI 和 AutoGen 做营销，创建后几个月内冲到约 1.6 万星。`,
			reason: `Edict 卖的是系统架构，读起来更像制度 cosplay。产品故事是：唐代官僚制才是更优的多智能体框架，有强制复核、奏折和军机处看板。隐喻底下仍是另一层 OpenClaw 编排皮：安装脚本、SOUL 提示词和仪表盘。公开争议是标本的一部分：早约 21 小时创建、同样朝廷框架并有重叠拼音 agent ID 的 wanikua/danghuangshang，提交了抄袭/未署名 issue #55 与 #94。Edict 以非技术讨论为由关闭了它们，星标继续涨。失败模式不是单一 CVE，而是角色扮演模板上的宣传密度、双仓戏剧，以及 homepage 指向 openclaw.ai、作品本身却是第三方 remix 的落差。现场还有看板不在线、agent 经常离线（#251）、webhook SSRF 加固这类运维债。`,
		},
		de: {
			description: `Edict verpackt OpenClaw-Multi-Agent-Workflows als Hof der Drei Departements und Sechs Ministerien: Prinzen-Routing, Planungs- und Prüfministerien und spezialisierte Bureau-Agenten hinter einem Echtzeit-Kanban. Das GitHub-Repo cft0808/edict ist Python-lastig, vermarktet sich hart gegen CrewAI und AutoGen und wuchs innerhalb weniger Monate auf etwa 16.000 Sterne.`,
			reason: `Edict verkauft Cosplay-Architektur als Systemdesign. Die Produktstory: Tang-Bürokratie sei ein überlegenes Multi-Agent-Framework mit Pflicht-Review, Memorialen und einem Militärkanban. Unter der Metapher sitzt eine weitere OpenClaw-Orchestrierungs-Haut mit Install-Skripten, SOUL-Prompts und Dashboard. Der öffentliche Streit gehört zum Specimen: wanikua/danghuangshang, etwa 21 Stunden früher erstellt, mit demselben Hof-Framing und überlappenden Pinyin-Agent-IDs, reichte Plagiats-/Attribution-Issues #55 und #94 ein. Edict schloss sie als nicht-technisch. Die Sterne stiegen weiter. Das Failure-Mode ist kein einzelnes CVE, sondern Hype-Dichte um ein Roleplay-Template, Twin-Repo-Drama und Homepage-Branding auf openclaw.ai bei einem Drittanbieter-Remix. Field-Issues umfassen Dashboard-offline-Meldungen, häufig offline Agenten (#251) und Webhook-SSRF-Härten.`,
		},
		it: {
			description: `Edict impacchetta workflow multi-agent OpenClaw come una corte dei Tre Dipartimenti e Sei Ministeri: routing del principe, ministeri di pianificazione e revisione, e agent di ufficio specializzati dietro una kanban in tempo reale. Il repo GitHub cft0808/edict è Python-heavy, si promuove contro CrewAI e AutoGen e in pochi mesi ha raggiunto circa 16k stelle.`,
			reason: `Edict vende architettura cosplay come system design. La storia di prodotto è che la burocrazia stile Tang è un framework multi-agent superiore, con review obbligatorie, memoriali e una kanban da affari militari. Sotto la metafora c’è un’altra pelle di orchestrazione OpenClaw con script di install, prompt SOUL e dashboard. La disputa pubblica fa parte dello specimen: wanikua/danghuangshang, creato circa 21 ore prima con lo stesso framing di corte e ID agent pinyin sovrapposti, ha aperto le issue di plagio/attribuzione #55 e #94. Edict le ha chiuse come discussioni non tecniche. Le stelle hanno continuato a salire. Il failure mode non è un singolo CVE: è densità di hype su un template role-play, drama da twin-repo e branding homepage verso openclaw.ai mentre il lavoro è un remix di terzi. In campo restano report di dashboard offline, agent spesso offline (#251) e hardening webhook SSRF.`,
		},
		ja: {
			description: `Edict は OpenClaw のマルチエージェント作業を三省六部の朝廷として包装します。太子ルーティング、計画・審査の「部」、リアルタイム kanban の奥に専門 agent がいます。GitHub の cft0808/edict は Python 中心で、CrewAI や AutoGen と強く対比して売られ、作成後数か月で約 1.6 万 star まで伸びました。`,
			reason: `Edict はシステム設計として売っていますが、中身は制度コスプレに近いです。製品ストーリーは、唐風官僚制が必須レビュー、奏折、軍機看板付きの優れたマルチエージェント枠だ、というものです。メタファーの下は、インストールスクリプト、SOUL プロンプト、ダッシュボード付きの別の OpenClaw オーケストレーション皮です。公開の争いは標本の一部です。約 21 時間早く作られ、同じ朝廷枠と重なる拼音 agent ID を持つ wanikua/danghuangshang が、剽窃・帰属の issue #55 と #94 を出しました。Edict は非技術議論として閉じ、star は増え続けました。失敗モードは単一 CVE ではなく、ロールプレイテンプレ周りのハイプ密度、双リポジトリのドラマ、homepage が openclaw.ai を指すのに中身は第三者 remix である落差です。現場にはダッシュボードオフライン、agent がしばしばオフライン（#251）、webhook SSRF 強化の課題も残ります。`,
		},
		ko: {
			description: `Edict는 OpenClaw 멀티 에이전트 워크플로를 三省六部 궁정으로 포장합니다. 태자 라우팅, 기획·검토 부처, 실시간 칸반 뒤의 전문 bureau 에이전트. GitHub 저장소 cft0808/edict는 Python 중심이고 CrewAI·AutoGen과 강하게 대비해 마케팅하며, 생성 후 수개월 만에 약 1.6만 스타까지 올랐습니다.`,
			reason: `Edict는 시스템 설계로 팔리지만 제도 코스프레에 가깝습니다. 제품 이야기는 당나라식 관료제가 필수 리뷰, 상소, 군기 칸반이 있는 우월한 멀티 에이전트 프레임이라는 것입니다. 은유 아래는 설치 스크립트, SOUL 프롬프트, 대시보드를 가진 또 다른 OpenClaw 오케스트레이션 껍질입니다. 공개 분쟁은 표본의 일부입니다. 약 21시간 먼저 만들어졌고 같은 궁정 프레임과 겹치는 병음 에이전트 ID를 가진 wanikua/danghuangshang가 표절·출처 이슈 #55와 #94를 올렸습니다. Edict는 비기술 토론으로 닫았고 스타는 계속 올랐습니다. 실패 모드는 단일 CVE가 아니라 롤플레이 템플릿 주변의 과열, 쌍둥이 저장소 드라마, homepage가 openclaw.ai를 가리키는데 작업물은 서드파티 remix인 간극입니다. 현장에는 대시보드 오프라인, 에이전트가 자주 오프라인(#251), webhook SSRF 강화 과제도 남아 있습니다.`,
		},
		es: {
			description: `Edict empaqueta flujos multi-agente de OpenClaw como una corte de Tres Departamentos y Seis Ministerios: enrutado del príncipe, ministerios de planificación y revisión, y agentes de oficina detrás de un kanban en tiempo real. El repo de GitHub cft0808/edict es mayormente Python, se promociona duro frente a CrewAI y AutoGen, y en meses llegó a unas 16k estrellas.`,
			reason: `Edict vende arquitectura de cosplay como diseño de sistemas. La historia de producto es que la burocracia al estilo Tang es un framework multi-agente superior, con revisiones obligatorias, memoriales y un tablero de asuntos militares. Bajo la metáfora hay otra piel de orquestación OpenClaw con scripts de instalación, prompts SOUL y dashboard. La disputa pública forma parte del specimen: wanikua/danghuangshang, creado unas 21 horas antes con el mismo marco de corte e IDs de agente pinyin solapados, abrió las issues de plagio/atribución #55 y #94. Edict las cerró como discusión no técnica. Las estrellas siguieron subiendo. El modo de fallo no es un solo CVE: es densidad de hype en torno a una plantilla de role-play, drama de repos gemelos y branding de homepage hacia openclaw.ai mientras el trabajo es un remix de terceros. En campo siguen reportes de dashboard offline, agentes a menudo offline (#251) y endurecimiento webhook SSRF.`,
		},
	},
	danghuangshang: {
		zh: {
			description: `当皇上（wanikua/danghuangshang）是一套 OpenClaw 教程与配置包，把 Discord 或飞书变成明制 AI 朝廷：太监传话、内阁、六部、都察院，以及可选的写小说 agent。宣传是一键登基、五分钟上手，18+ agent、60+ skills。`,
			reason: `问题不在 agent 能不能起好笑的官职名，而在于把提示词和配置包卖成治国术。安装脚本、朝代模板（明、唐、现代公司）和徽章墙承诺一支常驻的帝国班底。公开 issue 显示安装漂移、样例配置过期、Docker 起不来，以及 allowBots 设错时 Discord 多 bot 互刷风暴。项目还对同周出现的三省六部 OpenClaw 孪生仓 cft0808/edict 发起公开原创维权；两边都仍是同一 OpenClaw 网关上的 cosplay 皮。署名表演加上脆弱上手体验，就是质量签名。正文不裁定法律归属，只记录公开的同构指控与运行债。`,
		},
		de: {
			description: `Dang Huang Shang (wanikua/danghuangshang) ist ein OpenClaw-Tutorial- und Config-Paket, das Discord oder Feishu in einen Ming-Stil-KI-Hof verwandelt: Eunuchen-Relay, Kabinett, sechs Ministerien, Zensorat und optionale Roman-Agenten. Vermarktet als Ein-Kommando-Thronbesteigung in fünf Minuten mit 18+ Agenten und 60+ Skills.`,
			reason: `Das Specimen ist nicht, dass Agenten lustige Amtstitel tragen können. Es ist, dass ein Prompt-und-Config-Produkt als Staatskunst verkauft wird. Install-Skripte, Regime-Templates (Ming, Tang, moderne Firma) und Badge-Wände versprechen einen immer-an-imperialen Stab. Offene Issues zeigen Install-Drift, veraltete Sample-Configs, Docker das nicht startet und Discord-Multi-Bot-Stürme bei falschem allowBots. Das Projekt führt außerdem eine öffentliche Originalitätskampagne gegen cft0808/edict, einen Three-Departments/Six-Ministries-OpenClaw-Zwilling derselben Woche — beide bleiben Cosplay-Häute desselben OpenClaw-Gateways. Attribution-Theater plus brüchiges Onboarding ist die Qualitätssignatur. Der Text entscheidet keine Rechtsfrage; er hält den öffentlichen Isomorphie-Claim und die Runtime-Schuld fest.`,
		},
		it: {
			description: `Dang Huang Shang (wanikua/danghuangshang) è un tutorial e pacchetto di config OpenClaw che trasforma Discord o Feishu in una corte AI in stile Ming: staffetta eunuca, gabinetto, sei ministeri, censorato e agent opzionali da romanzo. Venduto come intronizzazione one-command in cinque minuti con 18+ agent e 60+ skill.`,
			reason: `Lo specimen non è che gli agent possano indossare titoli buffi. È che un prodotto di prompt e config viene venduto come arte di governo. Script di install, template di regime (Ming, Tang, azienda moderna) e muri di badge promettono uno staff imperiale always-on. Le issue aperte mostrano drift di install, sample config stantie, Docker che non parte e tempeste multi-bot Discord se allowBots è sbagliato. Il progetto conduce anche una campagna pubblica di originalità contro cft0808/edict, gemello OpenClaw Three Departments / Six Ministries della stessa settimana; entrambi restano pelli cosplay dello stesso gateway OpenClaw. Teatro di attribuzione più onboarding fragile è la firma di qualità. Il testo non adjudica la proprietà legale; registra la claim pubblica di isomorfismo e il debito runtime.`,
		},
		ja: {
			description: `当皇上（wanikua/danghuangshang）は OpenClaw のチュートリアル兼設定パックで、Discord や Feishu を明風の AI 朝廷にします。宦官リレー、内閣、六部、都察院、任意の小説 agent。売りはワンコマンド即位・五分で開始、18+ agent、60+ skills です。`,
			reason: `標本のポイントは、agent が面白い官職名を着られることではありません。プロンプトと設定の製品を統治術として売っていることです。インストールスクリプト、王朝テンプレ（明・唐・現代企業）、バッジ壁は常駐の帝国スタッフを約束します。公開 issue はインストールのずれ、古いサンプル設定、起動しない Docker、allowBots を誤ると起きる Discord マルチ bot ストームを示します。プロジェクトは同週の三省六部 OpenClaw 双生 cft0808/edict に対する公開のオリジナリティ運動も行い、双方は同じ OpenClaw ゲートウェイ上のコスプレ皮のままです。帰属の演劇と壊れやすいオンボーディングが品質の署名です。本文は法的所有を裁定せず、公開の同型主張とランタイム負債を記録します。`,
		},
		ko: {
			description: `당황상(wanikua/danghuangshang)은 Discord나 Feishu를 명나라 풍 AI 궁정으로 바꾸는 OpenClaw 튜토리얼·설정 팩입니다. 환관 릴레이, 내각, 육부, 도찰원, 선택적 소설 에이전트. 원커맨드 즉위, 5분 온보딩, 18+ 에이전트, 60+ 스킬로 팔립니다.`,
			reason: `표본의 핵심은 에이전트가 웃긴 관직을 달 수 있다는 점이 아닙니다. 프롬프트·설정 제품을 국정술처럼 판다는 점입니다. 설치 스크립트, 왕조 템플릿(명·당·현대 회사), 배지 벽은 상시 제국 스태프를 약속합니다. 공개 이슈는 설치 드리프트, 낡은 샘플 설정, 안 뜨는 Docker, allowBots가 틀릴 때 Discord 멀티 봇 폭풍을 보여 줍니다. 프로젝트는 같은 주의 三省六部 OpenClaw 쌍둥이 cft0808/edict를 상대로 공개 독창성 캠페인도 하며, 둘 다 같은 OpenClaw 게이트웨이 위의 코스프레 껍질로 남습니다. 출처 연극과 취약한 온보딩이 품질 서명입니다. 본문은 법적 소유를 판결하지 않고, 공개된 동형 주장과 런타임 부채를 기록합니다.`,
		},
		es: {
			description: `Dang Huang Shang (wanikua/danghuangshang) es un tutorial y paquete de config OpenClaw que convierte Discord o Feishu en una corte IA al estilo Ming: relevo de eunucos, gabinete, seis ministerios, censorado y agentes opcionales de novela. Se vende como entronización de un comando en cinco minutos con 18+ agentes y 60+ skills.`,
			reason: `El specimen no es que los agentes puedan llevar títulos graciosos. Es que un producto de prompts y config se vende como arte de gobierno. Scripts de instalación, plantillas de régimen (Ming, Tang, empresa moderna) y muros de badges prometen un staff imperial always-on. Issues abiertas muestran deriva de instalación, samples caducados, Docker que no arranca y tormentas multi-bot en Discord si allowBots está mal. El proyecto también hace una campaña pública de originalidad contra cft0808/edict, gemelo OpenClaw de Tres Departamentos y Seis Ministerios de la misma semana; ambos siguen siendo pieles cosplay del mismo gateway OpenClaw. Teatro de atribución más onboarding frágil es la firma de calidad. El texto no adjudica propiedad legal; registra la claim pública de isomorfismo y la deuda de runtime.`,
		},
	},
};
